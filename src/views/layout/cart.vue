      ...
      <div class="cart-item" v-for="item in cartList" :key="item.goods_id">
        ...
        <div class="info">
          ...
          <span class="bottom">
            ...
            <!-- 即希望保留原来的形参，又需要通过调用函数传参===可以通过箭头函数包装一层 -->
    ...
    goPay () {
      ...
      if (this.selCount > 0) {
        this.$router.push({
          ...
          query: {
            mode: 'cart',
            cartIds: this.selCartList.map(item => item.id).join(',')// map收集所有选中的商品的id,格式为[1,2,3,4,...]但是我们所需要的格式为字符串id之间用逗号隔开的形式‘1,2,3,4,....’ 所以用join
          }
        })
      }
    }
